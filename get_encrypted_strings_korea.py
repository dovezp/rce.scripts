...
def update_ms_aString():
    ...
    tmp_sp_addr = idc.FindBinary(0, SEARCH_DOWN, "8B 14 ?? ?? ?? ?? ?? 0F BE 02 6A 04") # 53 6F 72 72 79 2C 20 74 68 69 73 20 69 73 6E 27 74 20 61 20 62 61 63 6B 75 70 2E 20 4A 75 73 74 20 70 72 6F 6F 66 20 56 6B 69 6B 6F 32 20 6D 61 64 65 20 74 68 69 73 2E
    ...
    global ms_aString
    ms_aString = idaapi.get_long(tmp_sp_addr+0x3)
...
def decode_string(enc_arr, shift):
    ...
    return out_arr.tostring().decode("latin_1")#decode("cp949") ###########
...
def write_segment_strings(seg_start_ea, strs):
    ...
    for i in range(0, len(strs)):
        str_to_write = u"{}\x00".format(strs[i]).encode("utf_16")#encode("cp949") ######################
...
def main():
    ...
    for idx, call_addrs in sp_calls.iteritems():
        for addr in call_addrs:
            ...
            try_make_comment(addr, "(ID:{}) \"{}\"".format(idx, out_strings[idx].encode("utf_16", 'ignore')))#encode('cp949', 'ignore'))) ############################
